...
if [ "$METAL_CLUSTER_COUNT" -eq 0 ]; then
    local_pod=false
    SUBDOM="$CLUSTER_NAME-virt"
else
    local_pod=true
    SUBDOM=internal
fi
...
if $local_pod; then
    ...
    cp "$VIRT_FULLCHAIN_FILE" proxy/server.crt
    cp "$VIRT_PRIVATE_KEY_FILE" proxy/server.key
fi
for tpl in proxy/conf.d/*.tpl; do
    conf="$(echo "$tpl" | rev | cut -d. -f2- | rev)"
    < "$tpl" envsubst '$SUBDOM $BASE_DOMAIN' > "$conf"
done
...
if $local_pod; then
    ...
    sudo podman kube play --replace --network=host pod-local.yml
    ...
    podman build lab -t rhte-labguide --build-arg BUILD_REVISION="$(git rev-parse HEAD)"
    podman build proxy -t rhte-proxy
    cd "$ANSIBLE_DIR" || fail Unable to change into the ansible directory
    for image in labguide proxy; do
        podman save > "$DOWNLOAD_DIR/$image.tar" "rhte-$image"
        ansible metal -m copy -a "src=$DOWNLOAD_DIR/$image.tar dest=/tmp/"
        ansible metal -m shell -a "podman load < /tmp/$image.tar"
    done
    ansible metal -m copy -a "src=$SCRIPT_DIR/pod-remote.yml dest=~/pod.yml"
    ansible metal -m template -a "src=cm.yml.j2 dest=~/cm.yml"
    ansible metal -m shell -a 'if ! podman volume ls | grep -qF proxy-scratch; then podman volume create proxy-scratch; fi ; podman play kube --configmap ~/cm.yml --replace ~/pod.yml'
fi
